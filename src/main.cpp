...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        pchMessageStart[0] = 0xfc; //change last 2 digits on  first two
        pchMessageStart[1] = 0xc1;
        pchMessageStart[2] = 0xb7;
        pchMessageStart[3] = 0xdc;
        hashGenesisBlock = uint256("0x");  //new genesis block here
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nNonce   = 0;  //put new nonce here

        if (fTestNet)
        {
            block.nTime    = 1317798646;  //and again
            block.nNonce   = 0;  //and again 
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x"));  //delete code below after genesis block made
